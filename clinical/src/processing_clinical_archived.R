...
# withCallingHandlers(suppressWarnings(warning("Suppressing warnings")),
#                     warning = function(w) {
#   
# from the patient tracker spread sheet, load in every sheet as a df in a named list
suppressMessages(
clinical_list <- 
  clinicalbioinfoextensions::readxlAllSheets(xlsxFile = inClinical)
)
...
if("SNIPDx" %in% names(clinical_list)){
  ...
  to_keep <- c("Clinical", "Response", "Tumor markers", 
               "Dose Mods_Transfusions", "Genomics", "SNIPDx")
  
  print(paste("Keeping the following clinical data from the excel sheets:",
              to_keep))
  #clinical_list <- clinical_list[c(3,5:9)]
...
cols <- c(grep("Dose",colnames(clinical_list$Clinical),value = T),
...
suppressWarnings(
# increment through df list, if df col matches cols, convert cols class to numeric
for(i in seq_along(clinical_list)){
  for(j in seq_along(colnames(clinical_list[[i]]))){
    if(colnames(clinical_list[[i]][j]) %in% cols){
      clinical_list[[i]][,j] <- as.numeric(unlist(clinical_list[[i]][,j]))
    }
  }
}
...
if("SNIPDx" %in% names(clinical_list)){
  ...
  to_keep <- colsGenomic[colsGenomic %in% c("Patient ID", "Enrollment Gene","cDNA Change",
                                            ...
                                            "MSI","Reversion",  "Enrollment Test", "Enrollment Test Type", 
...
clinical_list$Clinical <- 
  clinicalbioinfoextensions::setGeneGroup(df = clinical_list$Clinical)
...
# colnames(BestResponse)[apply(BestResponse[,c(2:4)], 1, which.max)]
# 
...
clinical_list$BestMarkerResponseTime <- 
  clinical_list$BestMarkerResponseTime[,c(1,2,5)]
...
clinical_list$Clinical <- clinical_list$Clinical %>% mutate(COR = case_when(BOR == "PR"~"PR",
                                        PSA == "Response" ~"PR",
                                        `CA-125` == "Response" ~"PR",
                                        BOR == "SD"~"SD",
                                        BOR == "PD"~"PD"))
