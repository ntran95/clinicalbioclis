...
### correct sample id 
mmf$partner_sample_id <- as.character(mmf$partner_sample_id)
...
### correct sample id 
cnv$partner_sample_id <- as.character(cnv$partner_sample_id)
...
assay$`Date of Collection` <- as.Date(ifelse(is.na(assay$`Date of Collection`),
                                             yes = as.character(assay$`Sample Collection Date`),
                                             no = as.character(assay$`Date of Collection`)))
#remove random symbols (RP3500-01) in manifestqc to match variants
assay <- assay %>% mutate(`Sample ID` = gsub("\\*", "", `Sample ID`))
