...
### Nonclinical
params$currPatient <- "CCNE1_AMP" #test individual patient

params$currTrial <- "Non-Clinical"

params$inExtensions <- here::here("/ClinBio/SP-ClinicalBioinformatics/shared/git-repos/reparecompbio/clinicalbioinfoextensions/")

params$inInventory <- here::here("/ClinBio/SP-ClinicalBioinformatics/shared/SNiPDx/Non-Clinical/vendor-data/INVENTORY/rp_nonclin_genosity_inv_annotated_2022-09-30.csv")

params$inSamplesFolder <- here::here("/ClinBio/SP-ClinicalBioinformatics/shared/SNiPDx/Non-Clinical/vendor-data/dominik/Non-Clinical/")

params$inVendorData <- here::here("/ClinBio/SP-ClinicalBioinformatics/shared/SNiPDx/Non-Clinical/vendor-data/ARCHER/")
...
for(currSegment in inSegment){
  ...
  segmentLists[[currSample]] <- read_csv(currSegment) %>%
  ...
  for(sampleType in names(IDSplit[[patient]])){
    ...
    cat(sprintf("\n\n## %s\n\n", paste(sampleType)))
      ...
      #create sub table headers
        ...
        # order
        chrPaths <- chrPaths[order(chrPaths)]
